...
def main():
    ...
    try:
        ...
        # -------------------------------------------------------------------------
        phi_cable = 0
        with open(os.path.join(os.path.dirname(__file__), "phase-cable-calibration.yml"), "r") as phases_yaml:
            try:
                ...
                if HOSTNAME in phases_dict.keys():
                    ...
                    logger.error("Phase offset not found in phase-cable-calibration.yml")
        ...
        # -------------------------------------------------------------------------
        phi_offset = 0
        with open(os.path.join(os.path.dirname(__file__), "beam-weights.yml"), "r") as phases_yaml:
            try:
                phases_dict = yaml.safe_load(phases_yaml)
                if HOSTNAME in phases_dict.keys():
