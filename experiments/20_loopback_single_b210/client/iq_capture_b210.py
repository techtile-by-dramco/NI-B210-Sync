...
def setup(usrp, tx_gain, gain_db, exp_id, meas_id):
   ...
   # Generate a unique filename for the metadata
    ...
    for ch in TX_CHANNELS:
        usrp.set_tx_rate(RATE, ch)          # Set sample rate for the transmitter
        usrp.set_tx_gain(tx_gain, ch)       # Transmitter gain
    ...
    tx_streamer = usrp.get_tx_stream(st_args)       # TX streamer object
...
def main():
    ...
    # FPGA file path
    fpga_path = os.path.join(script_dir, "usrp_b210_fpga.bin")

    # Init USRP with local FPGA-file
