...
def compute_phase_difference(iq_data, fs):
    ...
    )  # This ensures phase is wrapped between -pi and pi
...
def store_phase_difference(in_dir, out_dir, fs):
    ...
    for filename in os.listdir(in_dir):
        if filename.endswith(".npy"):
            ...
            phase_differences_by_gain[(gain_b, frequency)].extend(phase_diff)
    ...
    with open(csv_filename, mode="w", newline="") as csvfile:
        ...
        for (gain, freq), phase_rad in phase_differences_by_gain.items():

            # Compute circmean and circstd for the current RX gain B
            circ_mean = circmean(
                phase_rad, high=np.pi, low=-np.pi
            )  # Circular mean in radians
            circ_var = (
                circstd(phase_rad, high=np.pi, low=-np.pi) ** 2
            )  # Circular variance
            circ_mean_deg = np.rad2deg(circ_mean)  # Convert to degrees
            circ_var_deg = np.rad2deg(
                np.sqrt(circ_var)
            )  # Convert variance to standard deviation (in degrees)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="./data/",
        help="Directory containing IQ data files and metadata (default: current directory)",
    )
    parser.add_argument(
        "--out-dir", type=str, default="./results/", help="Directory output"
    )
    parser.add_argument(
        "--fs", type=int, default=250000, help="Sampling frequency (Hz)"
    )
