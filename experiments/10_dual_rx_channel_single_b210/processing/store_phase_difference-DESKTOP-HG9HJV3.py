...
def store_phase_difference(in_dir, out_dir, fs):
    ...
    with open(csv_filename, mode='w', newline='') as csvfile:
     ...
     # Prepare CSV file to store the circmean and circstd for each gain value
    csv_filename = os.path.join(out_dir, "circmean_and_circstd_cat.csv")
    with open(csv_filename, mode='w', newline='') as csvfile:
        fieldnames = ['Category', 'Circular Mean (degrees)', 'Circular Std Dev (degrees)']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        ...
        for gain, phase_rad in phase_differences_by_gain.items():
            if gain > 33:
                phase_rad_above.extend(phase_rad)
            else:
                phase_rad_below.extend(phase_rad)

        # phase_rad_below = np.asarray(phase_rad_below).flatten()
        # phase_rad_above= np.array(phase_rad_above).flatten()

        for c, c_name in zip([phase_rad_below, phase_rad_above], ["below","above"]):

            # Compute circmean and circstd for the current RX gain B
            circ_mean = circmean(c, high=np.pi, low=-np.pi)  # Circular mean in radians
            circ_var = circstd(c, high=np.pi, low=-np.pi) ** 2  # Circular variance
            circ_mean_deg = np.rad2deg(circ_mean)  # Convert to degrees
            circ_var_deg = np.rad2deg(np.sqrt(circ_var))  # Convert variance to standard deviation (in degrees)

            # Write to CSV file
            writer.writerow({'Category': c_name, 'Circular Mean (degrees)': circ_mean_deg, 'Circular Std Dev (degrees)': circ_var_deg})
