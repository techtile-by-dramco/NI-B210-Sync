...
# Function to store phase difference data with TX gain and RX gain A, RX gain B
def store_phase_difference(in_dir, out_dir, fs):
    phase_differences_by_rx_gain = defaultdict(list)
    rx_gain_a_values = []
    rx_gain_b_values = []
    ...
    for filename in os.listdir(in_dir):
        if filename.endswith(".npy"):
            ...
            # Extract RX gain A and RX gain B from metadata
            ...
            # Append the phase difference for this RX gain A and RX gain B
            phase_differences_by_rx_gain[(rx_gain_a, rx_gain_b)].extend(phase_diff)

    # Prepare CSV file to store the circmean and circstd for each RX gain A and RX gain B
    csv_filename = os.path.join(out_dir, "circmean_and_circstd.csv")
    with open(csv_filename, mode='w', newline='') as csvfile:
        fieldnames = ['RX Gain A', 'RX Gain B', 'Circular Mean (degrees)', 'Circular Std Dev (degrees)']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()

        # Add data for each phase difference and RX gain A/B
        for (gain_a, gain_b), phase_rad in phase_differences_by_rx_gain.items():

            # Compute circmean and circstd for the current RX gain A and B
            ...
            # Write to CSV file
            writer.writerow({'RX Gain A': gain_a, 'RX Gain B': gain_b, 'Circular Mean (degrees)': circ_mean_deg, 'Circular Std Dev (degrees)': circ_var_deg})
...
def main():
    ...
    # Store phase difference data with fixed TX gain and variable RX gain A, RX gain B
