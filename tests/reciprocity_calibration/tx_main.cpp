...
int UHD_SAFE_MAIN(int argc, char *argv[])
{
        ...
        std::vector<sample_t> seq = read_ZC_seq(nsamps_per_buff, file);
        ...
        for (size_t ch = 0; ch < channel_nums.size(); ch++)
        {
                ...
                if (vm.count("gain"))
                {
                        ...
                        std::cout << boost::format("Actual Gain: %f ...") % usrp->get_tx_gain(channel)
                                  << std::endl
                                  << std::endl;
                }
        }
