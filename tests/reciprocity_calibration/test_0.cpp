...
int UHD_SAFE_MAIN(int argc, char *argv[])
{
        ...
        ("iq_port", po::value<std::string>(&port)->default_value("8888"), "Port to stream IQ samples to")("server-ip", po::value<std::string>(&server_ip), "SYNC server IP address")
        ("freq", po::value<double>(&freq)->default_value(400e6), "transmit RF center frequency in Hz")("rate", po::value<double>(&rate)->default_value(1e6), "rate of incoming samples")
        ("channels", po::value<std::string>(&channels)->default_value("0"), "which RX channel(s) to use (specify \"0\", \"1\", \"0,1\", etc)")("gain", po::value<double>(&gain)->default_value(0.5), "gain for the transmit RF chain")("ignore-server", po::bool_switch(&ignore_sync), "Discard waiting till SYNC server");
        ...
        std::vector<sample_t> seq(nsamps_per_buff, 0.8);      // read_ZC_seq(nsamps_per_buff, file);
